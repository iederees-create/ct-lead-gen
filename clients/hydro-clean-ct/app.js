/**
 * Hydro Clean — front-end logic.
 * Reads window.SITE_CONFIG (site-config.js) and renders all
 * config-driven content, plus wires up interactivity:
 * hamburger nav, FAQ accordion, quote-form validation, WhatsApp
 * message building, theme switching. The Surface Cleaning Estimate Planner
 * lives in its own module (planner.js).
 */
(function () {
  "use strict";
  ...
  function applyTheme(themeKey) {
    ...
    try { localStorage.setItem("ttc-theme", themeKey); } catch (e) { /* storage unavailable */ }
  }

  function initThemeSelector() {
    var saved = null;
    try { saved = localStorage.getItem("ttc-theme"); } catch (e) { /* ignore */ }
  ...
  function wireContactLinks() {
    ...
    $all("[data-whatsapp-link]").forEach(function (a) {
      var defaultMsg = "Hi " + CFG.business.name + ", I'd like a quote for a exterior cleaning project.";
    ...
    var areasSpan = $("#announcement-areas");
    if (areasSpan) areasSpan.textContent = (CFG.serviceAreas || []).slice(0, 3).join(", ");
  }
  ...
  function renderStats() {
    ...
    if (!wrap) return;
    wrap.innerHTML = "";
    CFG.trustIndicators.forEach(function (stat) {
  ...
  function renderQuoteServiceOptions() {
    var select = $("#quote-service");
    if (!select) return;
    (CFG.projectTypes || []).forEach(function (t) {
      var opt = el("option", null, escapeHtml(t));
      opt.value = t;
  ...
  function initQuoteForm() {
    ...
    function validate() {
      ...
      var service = $("#quote-service", form);
      ...
      if (!service.value) { showError(service, "Please select a project type."); valid = false; } else clearError(service);
    ...
    form.addEventListener("submit", function (e) {
      ...
      var service = $("#quote-service", form).value;
